...
def test(restore=True):
    ...
    X_train, X_test, y_train, y_test = train_test_split(images_selected, labels_encoded, test_size=0.30)
    X_train, X_val, y_train, y_val = train_test_split(images_selected, labels_encoded, test_size=0.30)
    ...
    predicted, actual, accuracy = model.test(batch_size=batch_size)

    print('Accuracy: {}'.format(accuracy))
    print('Predicted: {}'.format(predicted))
    print('Actual: {}'.format(actual))

    print(classification_report(actual, predicted, target_names=dataset.target_names))
